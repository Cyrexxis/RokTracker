...
try:
    ...
    print(
        "Could not find tesseract, please set the installation path manually in alliance-scanner.py (uncomment line 12 and set path to your executable)."
    )
    sys.exit(1)
else:
...
def add_results_to_sheet(governors, sheet, screen_number):
    ...
    for gov_num, governor in enumerate(governors):
        current_gov = (govs_per_screen * screen_number) + gov_num
        sheet.row_dimensions[current_gov + 2].height = 24.75
        # Write results in excel file
        sheet.add_image(
            OpImage(str(img_path / ("gov_name_" + str(current_gov + 1) + ".png"))),
            "A" + str(current_gov + 2),
        )
...
def governor_scan(mode: str, port: int, gov_number: int):
    ...
    while tries < max_tries:
        ...
        try:
            ...
            im_gov_name_bw = preprocessImage(
                im_gov_name,
                3,
                mode_data[mode]["threshold"],
                12,
                mode_data[mode]["invert"],
            )
            im_gov_name_bw_small = preprocessImage(
                im_gov_name,
                1,
                mode_data[mode]["threshold"],
                4,
            ...
            im_gov_score_bw = preprocessImage(
                im_gov_score,
                3,
                mode_data[mode]["threshold"],
                12,
            ...
            gov_name = pytesseract.image_to_string(
                im_gov_name_bw, config="--oem 1 --psm 7 -l eng"
            )
            gov_score = pytesseract.image_to_string(
                im_gov_score_bw, config="--oem 1 --psm 8"
            )
            ...
            if not reached_bottom:
                ...
                reached_bottom = True
                console.log(
                    "Didn't find score, assuming bottom is reached. Starting scan of last 6 governors."
                )
            else:
    ...
    return {"name": gov_name, "score": gov_score}
...
def scan(port: int, kingdom: str, mode: str, amount: int, resume: bool):
    ...
    for i in range(0, amount):
        ...
        if sheet1["B" + str(i + 1)].value == governor["name"] and sheet1[
            "C" + str(i + 1)
        ].value == to_int_check(governor["score"]):
            global reached_bottom
            reached_bottom = True
        ...
        table.add_column(
            "Value", str(datetime.timedelta(seconds=(amount - i) * 6)), style="cyan"
        )
